...
export const DeliveryDetailsPage: React.FC = () => {
  ...
  const [intercomPin, setIntercomPin] = useState("");
  ...
  const submitOrder = async () => {
    ...
    if (!apartment.trim() || !floor.trim() || !intercomPin.trim()) {
      window.alert("Заполните квартиру, этаж и пин от домофона.");
      return;
    }
    ...
    try {
      ...
      const detailedAddress =
        ...
        `Квартира: ${apartment.trim()}, этаж: ${floor.trim()}, домофон: ${intercomPin.trim()}`;
    ...
    <ScreenBackground>
      ...
      <section className="card section-card">
        ...
        <input
          className="search-input"
          value={intercomPin}
          onChange={(event) => setIntercomPin(event.target.value)}
          placeholder="Пин от домофона"
        />
